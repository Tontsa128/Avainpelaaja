...
export default function Page(){
 ...
 return <div className="min-h-screen bg-[#06101d]">
  ...
  {modal&&<Modal type={modal} sellers={mode==="work"?sellers:demoSellers} locations={mode==="work"?locations:demoLocations} close={()=>setModal(null)} onSave={async(data)=>{try{if(mode==="work"){if(modal==="seller"){const res:any=await api.createSeller({name:data.name,area:data.area,targetPerShift:8});setSellers(v=>[...v,{id:res.data.id,name:res.data.name,area:res.data.area||"Ei määritetty",shift:"—",target:res.data.targetPerShift||0,sales:0,status:"Varattu"}]);}else if(modal==="location"){const res:any=await api.createLocation({name:data.name,city:data.city,pricePerDay:Number(data.price)||0});setLocations(v=>[...v,{id:res.data.id,name:res.data.name,city:res.data.city,status:"Vapaa",price:Number(res.data.pricePerDay)||0,score:0,contact:"Ei määritetty"}]);}else if(modal==="booking"){const parts=String(data.date||"").split(".");const isoDate=parts.length===3?parts[2]+"-"+parts[1]+"-"+parts[0]:data.date;const res:any=await api.createBooking({startsAt:isoDate+"T"+(data.start||"10:00")+":00",endsAt:isoDate+"T"+(data.end||"18:00")+":00",seller:data.seller,location:data.location});const b=res.data;setBookings(v=>[...v,{id:b.id,date:new Date(b.startsAt).toLocaleDateString("fi-FI"),time:new Date(b.startsAt).toLocaleTimeString("fi-FI",{hour:"2-digit",minute:"2-digit"})+"–"+new Date(b.endsAt).toLocaleTimeString("fi-FI",{hour:"2-digit",minute:"2-digit"}),seller:b.seller?.name||data.seller||"Ei määritetty",location:b.location?.name||data.location||"Ei määritetty",status:"Odottaa"}]);}}else{if(modal==="seller")setSellers(v=>[...v,{id:Date.now(),name:data.name||"Uusi myyjä",area:data.area||"Ei määritetty",shift:"10:00–18:00",target:8,sales:0,status:"Varattu"}]);else if(modal==="location")setLocations(v=>[...v,{id:Date.now(),name:data.name||"Uusi kauppapaikka",city:data.city||"Ei määritetty",status:"Vapaa",price:Number(data.price)||0,score:0,contact:data.contact||"Ei määritetty"}]);else if(modal==="booking")setBookings(v=>[...v,{id:Date.now(),date:data.date||"Ei päivä",time:(data.start||"--:--")+"–"+(data.end||"--:--"),seller:data.seller||"Ei määritetty",location:data.location||"Ei määritetty",status:"Odottaa"}]);}setModal(null);notify("Tallennettu onnistuneesti");}catch(error){notify(error instanceof Error?error.message:"Tallennus epäonnistui");}}/>}
