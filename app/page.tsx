        ...
        try {
          if (mode === "work") {
            if (modal === "seller") {
              ...
              const res: any = await api.createBooking({ startsAt: `${isoDate}T${data.start || "10:00"}:00`, endsAt: `${isoDate}T${data.end || "18:00"}:00`, seller: data.seller, location: data.location });
              const b = res.data;
...
function Modal({type,sellers,locations,close,onSave}:{type:"booking"|"seller"|"location";sellers:Seller[];locations:Location[];close:()=>void;onSave:(d:any)=>void}){
 ...
 return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/75 p-4" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="w-full max-w-xl rounded-2xl border border-[#203451] bg-[#0b1829] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-bold">{title}</h2><button onClick={close} className="rounded-lg p-2 hover:bg-white/10"><X/></button></div><div className="grid gap-4 md:grid-cols-2">{type==="booking"?<><label className="text-sm text-slate-400">Myyjä<select value={data.seller||""} onChange={e=>setData({...data,seller:e.target.value})} className="mt-1 w-full rounded-xl border border-[#203451] bg-[#0d1a2c] p-3 text-white"><option value="">Valitse myyjä</option>{sellers.map(s=><option key={s.id} value={s.name}>{s.name}</option>)}</select></label><label className="text-sm text-slate-400">Kauppapaikka<select value={data.location||""} onChange={e=>setData({...data,location:e.target.value})} className="mt-1 w-full rounded-xl border border-[#203451] bg-[#0d1a2c] p-3 text-white"><option value="">Valitse kauppapaikka</option>{locations.map(l=><option key={l.id} value={l.name}>{l.name} · {l.city}</option>)}</select></label>{field("date","Päivämäärä","23.09.2026")}{field("start","Alkuaika","10:00")}{field("end","Loppuaika","18:00")}</>:type==="seller"?<>{field("name","Nimi")}{field("area","Alue")}</>:<>{field("name","Kauppapaikan nimi")}{field("city","Kaupunki")}{field("price","Hinta / päivä")}{field("contact","Yhteyshenkilö")}</>}</div><div className="mt-6 flex justify-end gap-2"><button onClick={close} className="btn-secondary">Peruuta</button><button onClick={()=>onSave(data)} className="btn-primary">Tallenna</button></div></div></div>
}
