...
export default function Page(){
 ...
 return <div className="min-h-screen bg-[#06101d]">
  ...
  {modal && (
    <Modal
      type={modal}
      sellers={mode === "work" ? sellers : demoSellers}
      locations={mode === "work" ? locations : demoLocations}
      close={() => setModal(null)}
      onSave={async (data) => {
        try {
          if (mode === "work") {
            if (modal === "seller") {
              const res: any = await api.createSeller({ name: data.name, area: data.area, targetPerShift: 8 });
              ...
              const selectedSeller = sellers.find((s) => String(s.id) === String(data.sellerId));
              const selectedLocation = locations.find((l) => String(l.id) === String(data.locationId));
              const res: any = await api.createBooking({
                startsAt: `${isoDate}T${data.start || "10:00"}:00`,
                endsAt: `${isoDate}T${data.end || "18:00"}:00`,
              ...
              const b = res.data;
              setBookings((v) => [...v, { id: b.id, date: new Date(b.startsAt).toLocaleDateString("fi-FI"), time: `${new Date(b.startsAt).toLocaleTimeString("fi-FI", { hour: "2-digit", minute: "2-digit" })}–${new Date(b.endsAt).toLocaleTimeString("fi-FI", { hour: "2-digit", minute: "2-digit" })}`, seller: b.seller?.name || data.seller || "Ei määritetty", location: b.location?.name || data.location || "Ei määritetty", status: "Odottaa" }]);
            }
          } else if (modal === "seller") {
            setSellers((v) => [...v, { id: Date.now(), name: data.name || "Uusi myyjä", area: data.area || "Ei määritetty", shift: "10:00–18:00", target: 8, sales: 0, status: "Varattu" }]);
          } else if (modal === "location") {
            setLocations((v) => [...v, { id: Date.now(), name: data.name || "Uusi kauppapaikka", city: data.city || "Ei määritetty", status: "Vapaa", price: Number(data.price) || 0, score: 0, contact: data.contact || "Ei määritetty" }]);
          } else if (modal === "booking") {
            setBookings((v) => [...v, { id: Date.now(), date: data.date || "Ei päivä", time: `${data.start || "--:--"}–${data.end || "--:--"}`, seller: data.seller || "Ei määritetty", location: data.location || "Ei määritetty", status: "Odottaa" }]);
          }
          setModal(null);
          notify("Tallennettu onnistuneesti");
        } catch (error) {
          notify(error instanceof Error ? error.message : "Tallennus epäonnistui");
        }
      }}
    />
  )}
...
function Calendar({bookings,open,notify}:{bookings:Booking[];open:any;notify:(x:string)=>void}){
 ...
 const moveDay=(delta:number)=>{const d=new Date(anchor);d.setDate(d.getDate()+delta);setAnchor(d);};
 ...
 return <div className="space-y-5">
  ...
  {view==="day"&&<div className="panel p-4"><div className="mb-4 flex items-center justify-between gap-2"><button onClick={()=>moveDay(-1)} className="btn-secondary">← Edellinen</button><h3 className="font-bold">{anchor.toLocaleDateString("fi-FI",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</h3><button onClick={()=>moveDay(1)} className="btn-secondary">Seuraava →</button></div><div className="space-y-2">{dayBookings(anchor).map(b=><button key={b.id} onClick={()=>notify(`Varaus: ${b.seller} / ${b.location}`)} className={`w-full rounded-xl border p-3 text-left ${statusClass(b.status)}`}><b>{b.time}</b> · {b.seller} · {b.location}</button>)}{dayBookings(anchor).length===0&&<EmptyState text="Tälle päivälle ei ole varauksia."/>}</div></div>}
  {view==="month"&&<div className="panel p-4"><div className="mb-3 grid grid-cols-7 gap-1 text-center text-xs font-semibold text-slate-500">{["Ma","Ti","Ke","To","Pe","La","Su"].map(x=><div key={x} className="p-2">{x}</div>)}</div><div className="grid grid-cols-7 gap-1">{monthDays.map(d=>{const current=d.getMonth()===anchor.getMonth();const count=dayBookings(d).length;return <button key={key(d)} onClick={()=>{setAnchor(d);setView("day");}} className={`min-h-24 rounded-lg border p-2 text-left text-xs transition hover:border-blue-500/50 ${current?"border-[#203451] bg-white/[.02]":"border-[#18283d] bg-black/10 text-slate-600"} ${key(d)===todayKey?"ring-1 ring-blue-500/60":""}`}><div className="flex items-center justify-between"><b>{d.getDate()}</b>{count>0&&<span className="rounded-full bg-blue-500/20 px-1.5 py-0.5 text-blue-200">{count}</span>}</div><div className="mt-2 space-y-1">{dayBookings(d).slice(0,2).map(b=><div key={b.id} className={`truncate rounded px-1.5 py-1 ${statusClass(b.status)}`}>{b.time} · {b.seller}</div>)}</div></button>})}</div></div>}
 </div>;
}

function CRM({notify,mode}:{notify:(x:string)=>void;mode:AppMode}) {
  const stageDefs = [
    ["NEW","Uusi"],["CALL","Soitettava"],["NEGOTIATION","Neuvottelu"],
    ["OFFER_SENT","Tarjous lähetetty"],["WAITING","Odottaa vastausta"],["AGREED","Vahvistettu"]
  ] as const;
  ...
  const load=async()=>{
    ...
    setLoading(true);
    try{
      const res=await api.crmOpportunities();
      setItems(res.data||[]);
    }catch(error){
      notify(error instanceof Error?error.message:"CRM-tietoja ei voitu ladata");
    }finally{
      setLoading(false);
    }
  };
  ...
  const create=async()=>{
    if(!form.name.trim() || !form.nextAction.trim()){
      notify("Nimi ja seuraava toimenpide ovat pakollisia");
      return;
    }
    try{
      const res=await api.createCrmOpportunity(form);
      setItems(v=>[res.data,...v]);
      setNewOpen(false);
      setForm({name:"",city:"",nextAction:"Soita",stage:"NEW",notes:""});
      notify("CRM-kohde tallennettu");
    }catch(error){
      notify(error instanceof Error?error.message:"CRM-kohteen tallennus epäonnistui");
    }
  };

  const move=async(id:string,stage:string)=>{
    try{
      const res=await api.updateCrmOpportunity(id,{stage});
      setItems(v=>v.map(x=>x.id===id?res.data:x));
      notify("CRM-vaihe päivitetty");
    }catch(error){
      notify(error instanceof Error?error.message:"Vaiheen päivitys epäonnistui");
    }
  };

  const addActivity=async(id:string,stage:string)=>{
    if(!activity.subject.trim()){
      notify("Aktiviteetin aihe puuttuu");
      return;
    }
    try{
      ...
      setActivity({type:"CALL",subject:"Puhelu",notes:""});
      notify("CRM-aktiviteetti tallennettu");
    }catch(error){
      notify(error instanceof Error?error.message:"Aktiviteetin tallennus epäonnistui");
    }
  };

  const demoCards=crmSeed.map(x=>({
    id:String(x.id),name:x.location,contact:x.contact,nextAction:x.next,notes:x.note,
    stage:stageDefs.find(s=>s[1]===x.stage)?.[0]||"NEW"
  }));
  ...
  return <div className="space-y-4">
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="text-sm text-slate-400">
        {mode==="demo" ? "Demo näyttää CRM-putken rakenteen." : "Työtilan CRM tallentuu PostgreSQL-tietokantaan API:n kautta."}
      </div>
    ...
    {mode==="work" && loading ? <EmptyState text="Ladataan CRM-tietoja..." /> : (
      <div className="grid gap-3 overflow-x-auto xl:grid-cols-6">
        {stageDefs.map(([stage,label])=>(
          <div key={stage} className="min-w-[245px] rounded-2xl border border-[#203451] bg-[#0d1a2c] p-3">
            <div className="mb-3 flex items-center justify-between">
              <div className="font-bold text-sm">{label}</div>
              <span className="rounded-full bg-white/5 px-2 py-1 text-xs text-slate-400">{cards.filter(x=>x.stage===stage).length}</span>
            </div>
            <div className="space-y-2">
              {cards.filter(x=>x.stage===stage).map(x=>(
                <div key={x.id} className="rounded-xl border border-[#203451] bg-white/[.02] p-3 text-sm">
                  <b>{x.name}</b>
                  <div className="text-xs text-slate-500">{x.city||x.contact||"Ei lisätietoa"}</div>
                  ...
                  {activityOpen===x.id && <div className="mt-3 rounded-xl border border-violet-500/20 bg-violet-950/10 p-3">
                    <select value={activity.type} onChange={e=>setActivity({...activity,type:e.target.value})} className="w-full rounded-lg border border-[#304968] bg-[#0d1a2c] p-2 text-xs">
                      <option value="CALL">Puhelu</option><option value="EMAIL">Sähköposti</option><option value="NOTE">Muistiinpano</option><option value="MEETING">Tapaaminen</option><option value="OFFER">Tarjous</option>
                    </select>
    ...
    {mode==="work" && newOpen && <div className="panel p-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <select value={form.stage} onChange={e=>setForm({...form,stage:e.target.value})} className="input">{stageDefs.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>
...
function Hours({sellers,locations,mode,notify}:{sellers:Seller[];locations:Location[];mode:AppMode;notify:(x:string)=>void}){
...
function Modal({type,sellers,locations,close,onSave}:{type:"booking"|"seller"|"location";sellers:Seller[];locations:Location[];close:()=>void;onSave:(d:any)=>void}){
 ...
 return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/75 p-4" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><div className="w-full max-w-xl rounded-2xl border border-[#203451] bg-[#0b1829] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-bold">{title}</h2><button onClick={close} className="rounded-lg p-2 hover:bg-white/10"><X/></button></div><div className="grid gap-4 md:grid-cols-2">{type==="booking"?<><label className="text-sm text-slate-400">Myyjä<select value={data.sellerId||""} onChange={e=>setData({...data,sellerId:e.target.value})} className="mt-1 w-full rounded-xl border border-[#203451] bg-[#0d1a2c] p-3 text-white"><option value="">Valitse myyjä</option>{sellers.map(s=><option key={s.id} value={String(s.id)}>{s.name}</option>)}</select></label><label className="text-sm text-slate-400">Kauppapaikka<select value={data.locationId||""} onChange={e=>setData({...data,locationId:e.target.value})} className="mt-1 w-full rounded-xl border border-[#203451] bg-[#0d1a2c] p-3 text-white"><option value="">Valitse kauppapaikka</option>{locations.map(l=><option key={l.id} value={String(l.id)}>{l.name} · {l.city}</option>)}</select></label>{field("date","Päivämäärä","23.09.2026")}{field("start","Alkuaika","10:00")}{field("end","Loppuaika","18:00")}</>:type==="seller"?<>{field("name","Nimi")}{field("area","Alue")}</>:<>{field("name","Kauppapaikan nimi")}{field("city","Kaupunki")}{field("price","Hinta / päivä")}{field("contact","Yhteyshenkilö")}</>}</div><div className="mt-6 flex justify-end gap-2"><button onClick={close} className="btn-secondary">Peruuta</button><button onClick={()=>onSave(data)} className="btn-primary">Tallenna</button></div></div></div>
}
